...
const bibleToday = async (
  ...
  const todayGuide = (await getTodayGuide(GuideModel)) as GuideInterface
  ...
  const pl = todayGuide.pl
  const pb = todayGuide.pb
  const alt = todayGuide.alt
