...
// SQLite Queries
import { queries } from './sqlite.native'
 ...
 */
export function ReadLocalDatabaseNativeProvider({
  children,
}: PropsWithChildren) {
  // States
 ...
 * Native only! (iOS + Android)
 */
export function useReadLocalDatabaseNative() {
  const value = useContext(ReadLocalDatabaseNativeContext)

  if (!value) {
    throw new Error(
      'useReadLocalDatabaseNative must be used within a ReadLocalDatabaseNativeProvider',
    )
  }
 ...
 * Native only! (iOS + Android)
 */
export function useReadLocalDatabaseMutationNative({
  onSuccess,
}: {
  onSuccess: () => void
}) {
